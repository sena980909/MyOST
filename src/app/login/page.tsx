...
function detectInAppBrowser(): boolean {
  ...
  // KakaoTalk, Instagram, Facebook, LINE, Naver, etc.
...
function openInExternalBrowser() {
  ...
  // iOS: Safari can't be forced from WebView easily,
  // but window.open sometimes works in some in-app browsers
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-purple-100 dark:border-purple-800 rounded-2xl p-6 shadow-sm">
          ...
          {isInApp ? (
            <div className="space-y-2">
              <div className="w-full px-4 py-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-xl text-center">
                <p className="text-amber-700 dark:text-amber-300 text-xs mb-2">
                  {t.login.inAppBrowser}
                </p>
                <button
                  onClick={openInExternalBrowser}
                  className="px-4 py-2 bg-amber-500 dark:bg-amber-600 text-white rounded-lg text-xs font-medium hover:bg-amber-600 dark:hover:bg-amber-700 transition-all"
                >
                  {t.login.inAppBrowserAction}
                </button>
                <p className="text-amber-600 dark:text-amber-400 text-[10px] mt-2">
                  {t.login.inAppBrowserOr}
                </p>
              </div>
            </div>
          ) : (
